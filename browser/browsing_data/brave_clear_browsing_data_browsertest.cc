...
class TestClearDataOnExit
 ...
 public:
  TestClearDataOnExit() : observer_(this) {
  ...
  void TearDownOnMainThread() override {
    // Cycle both the MessageLoop and the Cocoa runloop. One for each browser.
...
IN_PROC_BROWSER_TEST_F(TestClearDataOnExit, NoPrefsSet) {
...
IN_PROC_BROWSER_TEST_F(TestClearDataOnExit, VerifyRemovalMasks) {
...
IN_PROC_BROWSER_TEST_F(TestClearDataOnExit, VerifyRemovalCompletion) {
  // Clear everything.
  SetClearAll(browser()->profile()->GetPrefs());
...
class TestClearDataOnExitTwoBrowsers : public TestClearDataOnExit {
public:
  TestClearDataOnExitTwoBrowsers() : TestClearDataOnExit() {
    browsers_count_ = 2u;
  }
...
IN_PROC_BROWSER_TEST_F(TestClearDataOnExitTwoBrowsers, SameProfile) {
...
IN_PROC_BROWSER_TEST_F(TestClearDataOnExitTwoBrowsers, OneOTR) {
  // Delete browsing history on exit.
  browser()->profile()->GetPrefs()->SetBoolean(
    browsing_data::prefs::kDeleteBrowsingHistoryOnExit, true);
  // OTR is from the same profile, so expect a single call.
  SetExepectedRemoveDataCallCount(1);
...
IN_PROC_BROWSER_TEST_F(TestClearDataOnExitTwoBrowsers, OneOTRExitsLast) {
  // Delete browsing history on exit.
  browser()->profile()->GetPrefs()->SetBoolean(
    browsing_data::prefs::kDeleteBrowsingHistoryOnExit, true);
  // OTR is from the same profile, so expect a single call.
  ...
  // Close regular profile window: cleanup shouldn't happen until the OTR window
  // is closed as well.
...
IN_PROC_BROWSER_TEST_F(TestClearDataOnExitTwoBrowsers, OneGuest) {
  // Delete browsing history on exit.
  browser()->profile()->GetPrefs()->SetBoolean(
    browsing_data::prefs::kDeleteBrowsingHistoryOnExit, true);
...
IN_PROC_BROWSER_TEST_F(TestClearDataOnExitTwoBrowsers, OneGuestExitsLast) {
  // Delete browsing history on exit.
  browser()->profile()->GetPrefs()->SetBoolean(
    browsing_data::prefs::kDeleteBrowsingHistoryOnExit, true);
  // Guest sessions don't count, so expect a single call.
...
IN_PROC_BROWSER_TEST_F(TestClearDataOnExitTwoBrowsers, TwoProfiles) {
  // Delete browsing history on exit.
  browser()->profile()->GetPrefs()->SetBoolean(
      browsing_data::prefs::kDeleteBrowsingHistoryOnExit, true);
