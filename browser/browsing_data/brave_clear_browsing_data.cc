...
// Gets the count of sessions with the given |profile| whether they are on or
// of the record. Guest sessions aren't counted as they clean up after
// themselves.
...
void BraveClearBrowsingData::ClearOnExit(Profile* profile) {
  ...
  // Guest sessions clean up after themselves.
  if (profile->IsGuestSession())
    return;

  // If this is an off-the-record profile, check if the original needs to have
  // browsing data cleared.
  Profile* original_profile =
      (profile->IsOffTheRecord()) ? profile : profile->GetOriginalProfile();

  // Check if any settings to clear data on exit have been turned on.
  int remove_mask = 0;
  int origin_mask = 0;
  if (!GetClearBrowsingDataOnExitSettings(original_profile, remove_mask,
                                          origin_mask))
    return;

  // Check if this is the last browser for this profile.
  if (!IsLastActiveSessionForProfile(original_profile))
    return;

  // Remove data.
  content::BrowsingDataRemover* remover =
    content::BrowserContext::GetBrowsingDataRemover(original_profile);

  // If testing, then the test will do the actual removal and wait.
